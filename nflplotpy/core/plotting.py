...
from nflplotpy.matplotlib.artists import add_mean_lines, add_median_lines, add_nfl_logos
from nflplotpy.matplotlib.scales import apply_nfl_theme

from .colors import get_team_colors
from .utils import clean_team_abbreviations, validate_teams
...
def _plot_team_stats_matplotlib(
    data: pd.DataFrame,
    x: str,
    y: str,
    team_column: str,
    ...
    **kwargs,
    ...
    if show_logos:
        ...
        # Add logos
        add_nfl_logos(ax, teams, data[x].values, data[y].values, width=logo_size)
    else:
...
def _plot_team_stats_plotly(
    ...
    **kwargs,
    ...
    try:
        ...
        from nflplotpy.plotly.traces import (
            add_mean_lines,
            add_median_lines,
            create_team_scatter,
        )
    except ImportError:
        msg = "Plotly backend requires plotly to be installed"
...
def plot_player_comparison(
    ...
    **kwargs,
    ...
    if player_data.empty:
        msg = f"No data found for players: {players}"
        raise ValueError(msg)

    # Validate columns
    missing_cols = [col for col in metrics if col not in player_data.columns]
    if missing_cols:
        msg = f"Missing metric columns: {missing_cols}"
        raise ValueError(msg)
...
def _plot_player_comparison_matplotlib(
    ...
    **kwargs,
    ...
    msg = f"Unsupported plot_type: {plot_type}"
    raise ValueError(msg)
...
def plot_game_flow(
    pbp_data: pd.DataFrame, game_id: str, backend: str = "matplotlib", **kwargs
) -> plt.Figure | Any:
    ...
    if game_data.empty:
        msg = f"No data found for game_id: {game_id}"
        raise ValueError(msg)
    ...
    if backend == "matplotlib":
        ...
        ax.plot(
            range(len(game_data)),
            game_data.get("score_differential", [0] * len(game_data)),
        )
        ...
        return fig
    msg = "Plotly backend for game flow not yet implemented"
    raise NotImplementedError(msg)


def plot_season_standings(
    standings_data: pd.DataFrame, season: int, backend: str = "matplotlib", **kwargs
) -> plt.Figure | Any:
    ...
    if backend == "matplotlib":
        ...
        if teams and wins:
            colors = get_team_colors(teams, "primary")
            ax.barh(teams, wins, color=colors)
            ax.set_title(f"{season} Season Standings")
            ax.set_xlabel("Wins")

        return fig
    msg = "Plotly backend for standings not yet implemented"
    raise NotImplementedError(msg)
