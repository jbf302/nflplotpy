...
class NFLDataPlayerManager:
    ...
    def _load_data(self):
        ...
        try:
            ...
            missing_cols = [col for col in required_cols if col not in self._id_data.columns]
            if missing_cols:
                warnings.warn(f"Missing columns in nfl_data_py: {missing_cols}", stacklevel=2)
                return False
    ...
    def gsis_to_espn(self, gsis_id: str) -> Optional[str]:
        ...
        try:
            ...
            warnings.warn(f"Error converting GSIS to ESPN for {gsis_id}: {e}", stacklevel=2)
        ...
        try:
            ...
            if matches.empty:
                ...
                fuzzy_matches = self._id_data[
                    self._id_data['name'].str.lower().str.contains(normalized_name, na=False)
                ]
