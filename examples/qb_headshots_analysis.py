    ...
    if not NFL_DATA_AVAILABLE:
        ...
        sample_data = pd.DataFrame({
            'player_display_name': [
                ...
                'Tua Tagovailoa', 'Aaron Rodgers', 'Russell Wilson', 'Kyler Murray',
                'Jalen Hurts', 'Joe Burrow', 'Justin Herbert', 'Geno Smith'
            ],
            'recent_team': [
                'KC', 'BUF', 'BAL', 'DAL', 'MIA', 'NYJ', 'PIT', 'ARI',
                'PHI', 'CIN', 'LAC', 'SEA'
            ],
    ...
    try:
        ...
        for idx, row in qb_stats.iterrows():
            if pd.notna(row['passer_player_id']):
                ...
                player_info = get_player_info_by_id(row['passer_player_id'], id_type='gsis')
                if player_info['espn_id']:
                    qb_stats.at[idx, 'espn_id'] = player_info['espn_id']
                    qb_stats.at[idx, 'validated_name'] = player_info['name'] or row['player_display_name']
                else:
...
def create_qb_headshot_plot(qb_data):
    ...
    for idx, row in qb_data.iterrows():
        ...
        try:
            ...
            ax.scatter(x, y, c=team_color, s=200, alpha=0.8, edgecolors='white', linewidth=2)
            ax.annotate(player_name.split()[-1], xy=(x, y), xytext=(5, 5), 
