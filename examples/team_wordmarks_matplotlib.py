...
def create_conference_wordmark_charts(team_data):
    ...
    for conf_data, ax, conf_name, conf_color in conferences:
        ...
        ax.axhline(
            y=median_val, color=conf_color, linestyle="--", linewidth=2, alpha=0.7
        )
        ax.text(
            len(teams) - 1,
            median_val + 3,
            f"{conf_name} Median: {median_val:+.1f}",
            ha="right",
            va="bottom",
            color=conf_color,
            fontweight="bold",
        ...
        ax.set_title(
            f"{conf_name} 2024 Team Performance: Net Points per Win\n"
...
def create_summary_table(team_data):
    ...
    print(
        f"Range: {team_data['net_points_per_win'].min():+.1f} to {team_data['net_points_per_win'].max():+.1f}"
    )
